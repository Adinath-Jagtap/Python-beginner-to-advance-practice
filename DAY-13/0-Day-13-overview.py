...
# =============================================================================
'''
1. Make GET request to public API
   - Fetch data from JSONPlaceholder posts endpoint
   - Print the response status code
   - Display the first 5 posts with title and body

2. Parse JSON response from API
   - Fetch user data from JSONPlaceholder users endpoint
   - Extract and print: name, email, and city for each user
   - Handle nested JSON structure (address -> city)

3. Add headers to API request
   - Make request with custom User-Agent header
   - Add Content-Type and Accept headers
   - Print request headers from response object

4. Make POST request with data
   - Create new post with title, body, and userId
   - Send POST request to JSONPlaceholder
   - Print the created post with ID from response

5. Handle API errors with try-except
   - Make request to invalid URL
   - Catch ConnectionError, HTTPError, and Timeout
   - Print appropriate error messages for each error type

6. Extract specific data from nested JSON
   - Fetch users from JSONPlaceholder
   - Extract: name, company name, and geo coordinates (lat, lng)
   - Store in list of dictionaries with clean structure

7. Make multiple API calls in loop
   - Fetch posts 1 through 10 using loop
   - Add 0.5 second delay between requests
   - Count and print how many were fetched successfully

8. Save API response to JSON file
   - Fetch all users from JSONPlaceholder
   - Save response to 'users.json' with proper formatting
   - Read back and verify data was saved correctly

9. Add query parameters to request
   - Fetch comments for specific post (postId=1)
   - Limit results to 5 using _limit parameter
   - Print comment names and emails

10. Work with API that requires authentication
    - Use GitHub API to fetch user info (works without auth)
    - Add Authorization header with token (optional)
    - Handle rate limit errors appropriately
    - Alternative: Use any free API key from RapidAPI
'''